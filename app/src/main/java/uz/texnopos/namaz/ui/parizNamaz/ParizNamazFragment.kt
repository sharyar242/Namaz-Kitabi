...
import kotlinx.android.synthetic.main.pariz_namaz_fragment.*
...
class ParizNamazFragment: Fragment(R.layout.pariz_namaz_fragment),ParizNamazView {
    ...
    private fun allParizNamaz() {
        (requireActivity() as MainActivity).supportActionBar?.title =
            when (parizNamazId){
                1 ->"АЗАН , АЗАН ДУЎАСЫ, ТӘКБИР"
                2 ->"ТАҢ НАМАЗ"
                3 ->"ПЕСИН НАМАЗ"
                4 ->"НАМАЗЛЫГЕР"
                5 ->"ШАМ НАМАЗ"
                6 ->"ҚУПТАН ҳәм ЎИТР НАМАЗЫ"
                else->"Null"
            }
    }
    ...
    private fun createDynamicViewsNamaz(article: ParizNamaz) {
        ...
        for (i in 0 until textPair.size-1) {
            if (textPair[i].first < textPair[i].second) {
                ...
                linearLayoutParizNamazIshi.addView(textView)
            }
            if (imagePair[i].first < imagePair[i].second) {
                ...
                linearLayoutParizNamazIshi.addView(imageView)
            }
        }
